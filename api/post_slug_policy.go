...
type PostSlugPolicy struct {
	Policy
	*API
}

func (p PostSlugPolicy) Create(next phi.HandlerFunc) phi.HandlerFunc {
	postPolicy := &PostPolicy{API: p.API}
	return p.API.Authorization.Apply(next, "PostSlugController", "Create",
